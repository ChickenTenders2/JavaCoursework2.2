...
public class MyArrayQueue {
    ...
    public void enqueue(Object theElement) {
        if (numElements == queue.length) {
            Object[] newQueue = new Object[queue.length * 2]; // Doubles capacity after full
        ...
        int end = (front + numElements) % queue.length; // Calculate the end index
    ...
    public Object dequeue() {
        ...
        for (int i = 0; i < numElements - 1; i++) { 
            queue[i] = queue[i + 1]; // Shift remaining elements to the front
        }
        queue[numElements - 1] = null; // Clear the last element
        numElements--;
